    ...
    if ($status == 'yes' || $status == 'Yes') {
        ...
        <div id="mySidenav" class="sidenav">
            ...
            <a href="index.php" class="logo">
                <span style="color:red;font-size:25px">ISE</span>
            </a>
            <a href="profile.php">
                <?php echo $fname . " " . $lname . " (" . strtoupper($eid) . ")" ?>
            </a>
        ...
        if (isset($_GET['search'])) {
            ...
            if ($sql_search_result_check > 0) {
                $rowss = mysqli_fetch_assoc($sql_search_result);
        ...
        if (isset($searchid)) {
            ...
            if (isset($_GET['type'])) {
                if ($_GET['type'] == 'attendance') {
                    ...
                    $total_pre = ($total_classes > 0) ? ($present_total / $total_classes) * 100 : 0;
                    ?>
                    <div align="center" style="background-color: lightgray; padding: 10px;">
                        <?php
                        echo '<p><b>Total Classes: </b>' . $total_classes . ' &nbsp;&nbsp; <b>Total Present: </b>' . $present_total . '</p><p><b>Total Percentage: </b>' . $total_pre . '%</p>';
                        ?>
                    </div>
                    <table border="2" align="center" cellpadding="5px">
                        ...
                        while ($rows = mysqli_fetch_assoc($resulti)) {
                            ...
                            $eid = $rows['eid'];
                            $date = $rows['date'];
                            $color = ($status == 'p' || $status == 'P') ? "#d3d3d3" : "red";
                            $textcolor = ($status == 'p' || $status == 'P') ? "green" : "white";
                            $status = ($status == 'p' || $status == 'P') ? "Present" : "Absent";
                            ?>
                ...
                if ($_GET['type'] == 'marks') { ?>
                    <table border="2" align="center" cellpadding="5px">
                        ...
                        while ($rows = mysqli_fetch_assoc($resulti)) {
                            ...
                            $percentage = ($totalmarks > 0) ? ($marksobtain / $totalmarks) * 100 : 0;
                            $background_color = ($percentage >= 75) ? "green" : (($percentage <= 40) ? "red" : "#ff7b00");
                            ?>
